...
for secrets in v1.list_secret_for_all_namespaces().items:
    if secrets.metadata.name == 'fonkfe':
...
def thumbnail(event, context):
    ...
    if event['data']['EventName'] == "s3:ObjectCreated:Put" and bucket == 'uploads':
        ...
        try:
            ...
            print("Can't get object from the bucket: ", bucket, filename, tf.name)
            print err
